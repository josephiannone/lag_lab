...
const main = () => {
    ...
    function formatReferences(summary) {
        const len = Object.keys(summary.result).length
        const first = Object.keys(summary.result).slice(-4, -1)
        // console.log(first)

        summary.result = first.map(elem => {
            return summary.result[elem]
        })
        ...
        for (let refs in summary.result) {
            if (refs !== 'uids') {
                ...
                publication = HTMLpublication.replace('%data%', 'http://www.ncbi.nlm.nih.gov/pubmed/' + summary.result[refs].uid);
